...
run count $N$ (parameter `n_runs`, range 10-50 000, default 100), the algorithm
...
The project is hosted as a single GitHub repository. All automation
configuration lives under the `.github/` directory at the repository root:

- *`.github/workflows/ci.yml`*: The continuous integration workflow
  (unit tests, integration tests, coverage upload).
- *`.github/workflows/security.yml`*: Security scanning workflow
  (Bandit SAST, pip-audit CVE scan, Trivy container scan).
- *`.github/workflows/docs.yml`*: Documentation publishing workflow
  (compile Typst PDF, publish to GitHub Releases and GitHub Pages).
- *`.github/dependabot.yml`*: Dependabot configuration for automated
  dependency updates (@sec:dependabot).

SonarCloud static analysis is configured through `sonar-project.properties`
at the repository root. It points to `api/`, `db/`, and `frontend/` as
source directories and to `coverage.xml` (generated by CI) as the coverage
report path. Two GitHub Actions secrets are required: `CODECOV_TOKEN` for
uploading coverage reports to Codecov, and `SONAR_TOKEN` for SonarCloud
analysis.

=== Branch Strategy and Protection Rules

The branching model is described in @sec:vcs. In summary: `main` is the
single long-lived branch and is kept in a deployable state at all times.
New work is developed on short-lived branches named `feature/<topic>` or
`fix/<topic>`. A pull request targeting `main` is required before merging;
the CI workflow must pass before the merge is allowed.

This is enforced through GitHub's branch protection rules configured on
`main`:

- *Require a pull request before merging*: Direct pushes to `main` are
  blocked.
- *Require status checks to pass*: The `Tests (Python 3.13)` and
  `E2E tests (Firefox)` CI jobs must succeed before a pull request can
  be merged.
- *Require branches to be up to date before merging*: The head branch
  must be current with `main` at the time of merge.
