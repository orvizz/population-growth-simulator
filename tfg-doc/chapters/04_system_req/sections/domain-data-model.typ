...
#figure(
  {
    ...
    table(
      ...
      [`stochastic`],        [Boolean],         [`false` — deterministic (single matrix). `true` — stochastic (multiple matrices, random selection at each step).],
      ...
      [`result_history`],    [List[List[Float]]],[Population vector $bold(v)(t)$ at each time step $t = 0 dots n$.],
      [`matrices_snapshot`], [List[Matrix]],    [Copy of `matrix_a` arrays captured at run time; immutable thereafter.],
      [`matrix_sequence`],   [List[Integer]],   [Index into `matrix_ids` chosen at each step (stochastic only).],
