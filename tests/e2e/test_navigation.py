...
def test_page_loads(shiny_page: Page):
    ...
    - all four navbar tabs are visible
    ...
    expect(shiny_page.get_by_role("tab", name="Quasi-Extinction")).to_be_visible()
...
def test_tab_switching(shiny_page: Page):
    ...
    share the same ID.
    """
    for tab_name in ("My matrices", "Simulate", "Quasi-Extinction", "Browse matrices"):
